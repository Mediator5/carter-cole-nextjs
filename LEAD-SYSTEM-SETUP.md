...
2. Name it `Carter Cole & Associates`, set the timezone to Eastern and the
   currency to USD.
3. Choose **Web**, enter `cartercoleandassociates.com`.
...
| `purchase_started` | Workbook checkout opened |
...
### Your welcome sequence is independent of all this
...
`/book` on this site and `/contact` on SmartTaxIQ both show the live calendar.
...
> `https://cartercoleandassociates.com/api/webhooks/calendly?key=YOUR_CALENDLY_WEBHOOK_SECRET`
...
2. **Contact form** — submit it. Alert arrives; the row appears at
   `/admin/subscribers`.
3. **Phone link** — tap a number on a phone. `phone_click` shows in GA4
   Realtime within about 30 seconds.
4. **Booking** — book yourself a slot on `/book`. You should get Calendly's
