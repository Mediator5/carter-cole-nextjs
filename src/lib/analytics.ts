...
function conversionLabel(event: ConversionEvent) {
  const map: Partial<Record<ConversionEvent, string | undefined>> = {
    lead_checklist: process.env.NEXT_PUBLIC_ADS_LABEL_CHECKLIST,
 ...
 *
 * A phone call is the most valuable thing a local tax site produces and the
 * easiest to lose track of, because it leaves the website entirely. This is
