 ...
 * The mailing list.
 *
 * Supabase is the system of record and always has been: it holds the tokens
 * that gate the checklist download, the per-subscriber send history, the
 * contact submissions and the admin dashboard. Nothing here replaces it.
 ...
 *
 * The 5-email welcome sequence is unaffected by any of this. It runs from
 * `sequence.ts` and `/api/cron/dispatch` against Supabase, sending through
 * whichever mail provider `mailer.ts` is configured for, and does not depend
 * on the list provider having automation features at all.
 *
